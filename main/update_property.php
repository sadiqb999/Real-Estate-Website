...
if(isset($_POST['update'])){
   ...
   $update_listing = $conn->prepare("UPDATE `property` SET property_name = ?, address = ?, price = ?, type = ?, offer = ?, bhk = ?, bedroom = ?, bathroom = ?, room_floor = ?, lift = ?, security_guard = ?, garden = ?, power_backup = ?, parking_area = ?, gym = ?, description = ? WHERE id = ?");   
   $update_listing->execute([$property_name, $address, $price, $type, $offer, $bhk, $bedroom, $bathroom, $room_floor, $lift, $security_guard, $garden, $power_backup, $parking_area, $gym, $description, $update_id]);
