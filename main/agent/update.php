...
if(isset($_POST['submit'])){
   ...
   if(!empty($name)){
      $verify_name = $conn->prepare("SELECT * FROM `agents` WHERE name = ?");
   ...
   if($old_pass != $empty_pass){
      if($old_pass != $prev_pass){
         ...
         if($new_pass != $empty_pass){
            $update_password = $conn->prepare("UPDATE `agents` SET password = ? WHERE id = ?");
            $update_password->execute([$c_pass, $agent_id]);
