...
if(isset($_POST['submit'])){
   ...
   $select_agents = $conn->prepare("SELECT * FROM `agents` WHERE name = ?");
   $select_agents->execute([$name]);
   ...
   if($select_agents->rowCount() > 0){
      ...
      if($pass != $c_pass){
         ...
         $insert_agent = $conn->prepare("INSERT INTO `agents`(id, name, password) VALUES(?,?,?)");
         $insert_agent->execute([$id, $name, $c_pass]);
